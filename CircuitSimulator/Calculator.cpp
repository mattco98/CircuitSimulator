...
bool Calculator::calculate(spot_vec spots, std::vector<Component*> components) {
    ...
    // If the current is not complete, or the grid is completely empty,
    ...
    std::vector<GridSpot*>::iterator it = populatedSpots.begin();

    // Loop through populated spots and assign them to a Node.
    while (it != populatedSpots.end()) {
        bool seen = false;
